...
bool							find_ban_user(std::vector<std::string> vec, std::string id)
{
	...
	{
		if (star_name_checker(id.c_str(), itr->c_str(), (int)id.length(), (int)itr->length()))
		{
			return true;
		}
	}
...
bool							check_name_match(user & target, user & member , std::string pattern)
{
	...
	if (pattern == "0" || pattern == "*")
		...
		return false;
	else if (star_name_checker(member.get_nick().c_str(), pattern.c_str(), (int)member.get_nick().length(), (int)pattern.length()))
		return true;
	else if (star_name_checker(member.get_hostname().c_str(), pattern.c_str(), (int)member.get_hostname().length(), (int)pattern.length()))
		return true;
	else if (star_name_checker(member.get_username().c_str(), pattern.c_str(), (int)member.get_username().length(), (int)pattern.length()))
		return true;
	else
		return false;
}
